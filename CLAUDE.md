...
Schema changes live as SQL files in `supabase/migrations/` (`001`–`007`). **The user runs migrations manually in the Supabase Studio SQL Editor. The agent never applies migrations and never touches the production database directly — it only creates or edits the `.sql` files.** `007_update_client_email.sql` adds `public.update_client_email(p_old_email, p_new_email)`, a `SECURITY INVOKER` function grantable only to `service_role` (see Clientes below).
...
- **Clientes** (`/admin/clientes`): CRM over the `clients` table (source of truth for who is a client) with per-client stats computed from `appointments` by email (sessions, billed total, last session, top two services; cancelled/no-show excluded). **`clients` and `appointments` are related only by the `email` / `email_cliente` text columns — there is no `client_id` or foreign key.** Search by name/email, create, edit name/phone/**email**/**private notes** (LFPDPPP notice), delete (removes only the `clients` row — its appointments stay). Editing the email calls `updateClient` (`src/app/actions/admin.ts`), which updates name/phone/notes directly and, only if the email changed, calls `update_client_email` (migration `007`, `supabase.rpc`) — a `SECURITY INVOKER` Postgres function grantable only to `service_role` — to rename `clients.email` **and** every matching `appointments.email_cliente` row in one atomic transaction, so the client's history/stats follow the new email instead of orphaning. `ClientesClient.tsx` keys the list by `clients.id` (not email) so the edited card survives the post-save refresh without remounting.
